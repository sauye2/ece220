 ...
 *The purpose of this mp is to create a code that slices a floorplan through repetiively cutting the floorpan horizontally or vertically.
 *In order to successfully implement this code, we will need to code the init_slicing_tree, get_total_resource, is_leaf_node, is_internal_node,
 *is_in_subtree, postfix_traversal, rotate, recut, swap_module, and swap_topology functions and make sure that they work correctly through testing. 
 *We were able to test by making use of the "make floorplanner" command as well as the "make circuit#" code to assess how we did on each of the 10
 *functions.By using the GDB, we were able to debug each part of the code that went wrong. 
 */
...
int is_leaf_node(node_t* ptr) {
  ...
  if(ptr->module!=NULL)                        //node is leaf if module is NULL
    return 1;
  else
    return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  if(ptr->cutline!=UNDEFINED_CUTLINE)             //node is internal if cutline is undefined_cutline
    return 1;
  else
    return 0;
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  for(b=b; b!=NULL;b=b->parent){           //start at node b and traverse the tree upwards
      if(b==a)
      return 1;
  }
  return 0;
...
void rotate(node_t* ptr) {
  // TODO: 
  node_t* temp=(node_t*)malloc(sizeof(node_t));          //allocate a temporary node
  temp->module->w = ptr->module->w;                      //swap width and height in the module
  ptr->module->w = ptr->module->h;
  ptr->module->h = temp->module->w;
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
    ptr->cutline = (cutline_t)!ptr->cutline;    //logical not the cutline to switch from vertical to horizontal or vice versa
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  node_t* temp=(node_t*)malloc(sizeof(node_t));               //allocate a temporary node
  temp->module=a->module;                                     //swap the two modules in a and b
  a->module=b->module;
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
    ...
    //change a_parent
    if(a_parent->left==a)   //if a then swap to b for left
        a_parent->left=b;

    if(a_parent->right==a)  //if a then swap to b for right
        a_parent->right=b;

    //change b_parent
    if(b_parent->left==b)   //if b then swap to a
        b_parent->left=a;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
    ...
    if(ptr->module!=NULL){                                     //assign module to expression if node has a module
        expression[*nth].module=ptr->module;
    }
    else if(ptr->cutline!=UNDEFINED_CUTLINE){                  //assign cutline to expression if node has a cutline
...
int get_total_resource(node_t* ptr)
{
  int count=0;
  int module=0;
  node_t* nodeptr=ptr->right;
  module_t* moduleptr=nodeptr->module;
  while(1){
    count = count+moduleptr->resource; //records resource cost
    module++; 
    if((module+1)==num_modules){ //if last internal node
      nodeptr=ptr->left; 
      moduleptr=nodeptr->module; //updates module pointer
      count = count+moduleptr->resource; //once again records cost
      break; 
    }
    nodeptr=ptr->left; 
    ptr=nodeptr; //update pointer
    nodeptr=ptr->right; 
    moduleptr=nodeptr->module; //update module pointer
  }
  return count;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
 ...
 node_t* ptr = (node_t*)malloc(sizeof(node_t));
    //Base case if last module
    if(n==num_modules-1){
      ptr->left=NULL; //there should be no siblings if last node
      ptr->right=NULL;
      ptr->parent=par;
      ptr->cutline=UNDEFINED_CUTLINE; //leaf node so undefined cutline
    ...
    //Right child
    ptr->right=(node_t*)malloc(sizeof(node_t)); //Allocates memory for right child
    ptr->right->right=NULL;
    ptr->right->left=NULL;
    ptr->right->module=&modules[n];
    ptr->right->cutline=UNDEFINED_CUTLINE; //creating new leaf node so undefined
    ptr->right->parent=ptr; //parent is pointer
